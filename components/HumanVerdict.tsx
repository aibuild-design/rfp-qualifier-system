...
export function HumanVerdict({
  ...
  if (current && !open) {
    const agrees = current === computed;
    return (
      <div className="mt-4 rounded-lg border border-rfp-border bg-rfp-surface-sunken px-4 py-3">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <p className="text-sm text-rfp-ink">
            <span className="font-semibold">Your call: {VERDICT_META[current].label}</span>
            <span className="text-rfp-ink-muted">
              {agrees ? " - same as the desk" : ` - the desk said ${VERDICT_META[computed].label}`}
            </span>
          </p>
          <button
            onClick={() => setOpen(true)}
            className="press text-xs font-medium text-rfp-ink-muted underline underline-offset-2 hover:text-rfp-ink"
          >
            Change
          </button>
        </div>
        {currentNote && <p className="mt-1.5 text-[13px] leading-relaxed text-rfp-ink-secondary">{currentNote}</p>}
        {decidedAt && (
          <p className="mt-1 text-[11px] text-rfp-ink-muted">
            Recorded {new Date(decidedAt).toLocaleDateString()}
          </p>
        )}
      </div>
    );
  ...
  if (!open) {
    return (
      <button
        onClick={() => setOpen(true)}
        className="press mt-4 text-sm font-medium text-rfp-ink-secondary underline underline-offset-2 hover:text-rfp-ink"
      >
  ...
  return (
    <div className="mt-4 rounded-lg border border-rfp-border bg-rfp-surface-sunken p-4">
      <p className="text-sm font-semibold text-rfp-ink">What would you have decided?</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button onClick={() => setOpen(false)} disabled={pending} className={buttonSecondaryClass}>
          Cancel
        </button>
        {current && (
